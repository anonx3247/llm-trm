...
class LatentAttentionCompressor(nn.Module):
    ...
    def __init__(
        ...
        params = (
            num_latents * hidden_size +  # Latent queries
            4 * hidden_size**2 +  # Attention weights
            8 * hidden_size**2  # Feed-forward
        )
        
        print(f"  Latent attention compression: any_length → {num_latents}")
        print(f"  Compression params: {params:,} ({params/1e6:.2f}M)")
