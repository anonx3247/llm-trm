...
Usage:
    # With compressor (default, 256 dim):
    ...
    # Without compressor (direct, 2048 dim):
    python -m src.train.phase2_trm \\
        --data_path ./data/hidden_pairs/hidden_pairs.pt \\
        --no_compressor \\
        --output_dir ./checkpoints/phase2_direct \\
        --num_epochs 100
"""
...
class Phase2Config:
    ...
    d_compressed: int = 256  # Must match compressor (ignored if use_compressor=False)
...
class DirectHiddenStateSequenceDataset(Dataset):
    """Dataset for variable-length hidden state sequences WITHOUT compression."""
    ...
    def __init__(
        ...
        Args:
            ...
            hidden_post: Target states [N, D]
        """
        self.hidden_pre_list = hidden_pre_list
        self.hidden_post = hidden_post

        print(f"Direct dataset: {len(hidden_pre_list)} samples, hidden_size={hidden_post.shape[-1]}")

        # Validate data
        for i, seq in enumerate(hidden_pre_list):
            if torch.isnan(seq).any() or torch.isinf(seq).any():
                print(f"Warning: NaN/Inf in hidden_pre[{i}]")
        if torch.isnan(hidden_post).any():
            print("Warning: NaN in hidden_post")

        # Print data statistics
        all_pre = torch.cat(hidden_pre_list, dim=0)
        print(
            f"  Pre stats: mean={all_pre.mean():.4f}, std={all_pre.std():.4f}, "
            f"min={all_pre.min():.4f}, max={all_pre.max():.4f}"
        )
        print(
            f"  Post stats: mean={hidden_post.mean():.4f}, "
            f"std={hidden_post.std():.4f}, "
            f"min={hidden_post.min():.4f}, max={hidden_post.max():.4f}"
        )

    def __len__(self) -> int:
        return len(self.hidden_pre_list)

    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            context: Context sequence [L, D]
            target: Target state [D]
        """
        return self.hidden_pre_list[idx], self.hidden_post[idx]
...
class TRMSequenceTrainer:
    ...
    def _load_compressor(self) -> None:
        """Load pre-trained compressor from Phase 1, or skip if use_compressor=False."""
        if not self.config.use_compressor:
            # Direct mode: no compressor, TRM operates on full hidden dim
            self.compressor = None
            # d_compressed = hidden_size in direct mode
            self.config.d_compressed = self.config.hidden_size
            print(f"Direct mode: TRM operates on full hidden dim ({self.config.hidden_size})")
            print("  No compressor loaded (use_compressor=False)")
            return
    ...
    def _init_wandb(self) -> None:
        ...
        mode_suffix = "direct" if not self.config.use_compressor else f"d{self.config.d_compressed}"
        run_name = self.config.wandb_run_name or f"phase2_seq_{mode_suffix}"
        wandb.init(
    ...
    def _load_data(self) -> DataLoader:
        ...
        if self.config.use_compressor:
            # Compressed mode: pre-compress sequences
            assert self.compressor is not None, "Compressor required when use_compressor=True"
            dataset: Dataset[tuple[torch.Tensor, torch.Tensor]] = HiddenStateSequenceDataset(
            ...
            # Direct mode: use raw hidden states
            dataset = DirectHiddenStateSequenceDataset(
                hidden_pre_list=hidden_pre_list,
                hidden_post=hidden_post,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Direct mode: TRM operates on full 2048 hidden dim (no compression)",
    )
    parser.add_argument(
        "--n_heads",
        type=int,
    ...
    # Default n_heads based on compression mode
    # With compressor (256 dim): 8 heads (32 dim per head)
    # Without compressor (2048 dim): 16 heads (128 dim per head)
    if args.n_heads is None:
        n_heads = 16 if args.no_compressor else 8
    else:
