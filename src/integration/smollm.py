...
from src.trm import TinyRecursiveNetwork, RecursiveReasoningBase, LatentAttentionCompressor
...
if __name__ == "__main__":
    print("="*70)
    print("Creating SmolLMv3 + TRM with Sliding Window Architecture")
    print("="*70)
    
    # Create model with LoRA and sliding window TRM
    model = create_smollm_trm_model(
        model_name="HuggingFaceTB/SmolLM3-3B",
        use_lora=True,
        lora_r=16,
        num_latents=256,  # Compress 65k → 256 (256x compression!)
        trm_kwargs={
            "n_layers": 2,
            "n_latent_steps": 4,
            "n_deep_recursions": 2,
    ...
    print(f"- Context length: 65,536 tokens")
    print(f"- Compression: variable_length → 256 latents")
    print(f"- Output: Sliding window (drop first 256, append 256 TRM states)")
    ...
    print("- <think> token triggers TRM reasoning in hidden space")
    print("- TRM generates M reasoning states (not text tokens)")
    print("- Sliding window: reasoning states replace oldest positions")
    print("- Model generates answer conditioned on TRM reasoning")
    print("\nNo expansion network needed - simpler and more elegant!")
